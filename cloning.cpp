    ...
    for (int j = 0; j < w; j++)
      for (int c = 0; c < channel; ++c) {
        gx.at<float>(i, j * channel + c) =
            (float)img.at<uchar>(i, (j + 1) * channel + c) -
            (float)img.at<uchar>(i, j * channel + c);
      }
}
    ...
    for (int j = 0; j < w; j++)
      for (int c = 0; c < channel; ++c) {
        gy.at<float>(i, j * channel + c) =
            (float)img.at<uchar>((i + 1), j * channel + c) -
            (float)img.at<uchar>(i, j * channel + c);
      }
}
    ...
    for (int j = 0; j < w - 1; j++)
      for (int c = 0; c < channel; ++c) {
        gxx.at<float>(i, (j + 1) * channel + c) =
            (float)img.at<float>(i, (j + 1) * channel + c) -
            (float)img.at<float>(i, j * channel + c);
      }
}
    ...
    for (int j = 0; j < w; j++)
      for (int c = 0; c < channel; ++c) {
        gyy.at<float>(i + 1, j * channel + c) =
            (float)img.at<float>((i + 1), j * channel + c) -
            (float)img.at<float>(i, j * channel + c);
      }
}
...
void Cloning::dst(double *mod_diff, double *sineTransform, int h, int w) {
  ...
  for (int i = 0; i < w; i++) {
    ...
    Mat planes1[] = {Mat::zeros(result.size(), CV_32F),
                     Mat::zeros(result.size(), CV_32F)};
...
void Cloning::solve(const Mat &img, double *mod_diff, Mat &result) {
  ...
  for (int i = 0; i < w - 2; i++, cy++) {
    for (int j = 0, cx = 1; j < h - 2; j++, cx++) {
      idx = j * (w - 2) + i;
      denom[idx] = (float)2 * cos(CV_PI * cy / ((double)(w - 1))) - 2 +
                   2 * cos(CV_PI * cx / ((double)(h - 1))) - 2;
    }
  }
...
void Cloning::poisson_solver(const Mat &img, Mat &gxx, Mat &gyy, Mat &result) {
  ...
  rectangle(bound, Point(1, 1), Point(img.cols - 2, img.rows - 2),
            Scalar::all(0), -1);
  ...
  for (int i = 1; i < h - 1; i++)
    for (int j = 1; j < w - 1; j++) {
      idx = i * w + j;
      boundary_point[idx] =
          -4 * (int)bound.at<uchar>(i, j) + (int)bound.at<uchar>(i, (j + 1)) +
          (int)bound.at<uchar>(i, (j - 1)) + (int)bound.at<uchar>(i - 1, j) +
          (int)bound.at<uchar>(i + 1, j);
    }
...
void Cloning::normal_clone(Mat &I, Mat &mask, Mat &wmask, Mat &cloned,
                           int num) {
  int w = I.size().width;
  int h = I.size().height;
  ...
  if (num == 1) {
    ...
    for (int i = 0; i < h; i++)
      for (int j = 0; j < w; j++) {
        if (abs(sgx.at<float>(i, j) - sgy.at<float>(i, j)) >
            abs(grx.at<float>(i, j) - gry.at<float>(i, j))) {
...
void Cloning::local_color_change(Mat &I, Mat &mask, Mat &wmask, Mat &cloned,
                                 float red_mul = 1.0, float green_mul = 1.0,
                                 float blue_mul = 1.0) {
  initialization(I, mask, wmask);
...
void Cloning::illum_change(Mat &I, Mat &mask, Mat &wmask, Mat &cloned,
...
void Cloning::texture_flatten(Mat &I, Mat &mask, Mat &wmask,
                              double low_threshold, double high_threshold,
                              int kernel_size, Mat &cloned) {
  initialization(I, mask, wmask);
